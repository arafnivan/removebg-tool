...
export function useBackgroundRemoval() {
  ...
  const run = useCallback(async (image, model) => {
    const id = ++runId.current;
    setStatus({ kind: "running", stage: null });
    try {
      const blob = await removeImageBackground(image.file, model, (stage) => {
      ...
      setResult({ sourceId: image.id, model, blob, url, ...size });
      setStatus({ kind: "idle" });
    } catch (error) {
  ...
  return {
    ...
    status,
    start,
    rerun: (model) => source && run(source, model),
    cancel,
    reset,
  };
}
